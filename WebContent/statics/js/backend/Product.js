...
function checkProduct() {
	...
	$.ajax({
		url : contextPath + "/ProductServlet?action=getImgs",
		method : "post",
		data : formData,
		contentType : false,
		processData : false,
		success : function(data) {
			...
			if(result.message=="添加成功!") { //添加成功!
				...
				//返回商品显示列表
				$(".m_right").load(contextPath+"/ProductServlet?action=index .m_right>*");
			}else if(result.message=="修改成功!"){ //修改成功
				mizhu.toast("操作成功",1500);
				//得到在哪一个修改
				var currentPage = $("#currentPage").val();
				//返回商品显示列表
				$(".m_right").load(contextPath+"/ProductServlet?action=index&currentPage="+currentPage+" .m_right>*");
			}else{
...
function change1() {
	...
	$("#productCategoryLevel2").load(contextPath + "/ProductCategoryService #productCategoryLevel2>*",item, leve2);
}
...
function leve2() {
	...
	$("#productCategoryLevel3").load(
			contextPath + "/ProductCategoryService #productCategoryLevel3>*",
			item);
}

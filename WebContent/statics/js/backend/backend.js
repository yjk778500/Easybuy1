	...
	$.ajax({
		...
		success : function(jsonStr) {
			...
			if (result.status == 1) {
				...
				$(".m_right").load(contextPath+ "/ProductServlet?action=index .m_right>*",ise);
			}
		}
	});
...
function s_btn(thisId, currentPage) {
	...
	if (identityCode == "" || identityCode == null) {
		...
		//判断用户名是否存在
		$.ajax({
			url		:		contextPath+"/EasybuyUserServlet?action=loginNameCount&name="+name,
			method	:		"post",
						...
						$.post(contextPath + "/EasybuyUserServlet?action=modify", item,
							function(data) {
								...
								if (result.message == "添加成功!") {  //添加成功!
									...
									//添加成功后跳回用户显示页面
									$(".m_right").load( contextPath + "/EasybuyUserServlet .m_right>*", "action=user");
...
function UpdatePwd(){
	...
	if($oldpwd == ""){
		...
		$.ajax({
			...
			method		:		"post",
			data		:		{"loginName":$loginName,"password":$newpwd,"action":"modifyPassWord"},
			success		:		function(data){
				var json = eval('('+data+')');
				if(json.status == 1){
					//alert(json.message);
					//跳转至登录页面
					location.href=contextPath+"/EasybuyUserServlet?action=forgetPwd";
				}
			}
...
function checkOrder(orderId,currentPage,isall,userId){
	mizhu.confirm('温馨提醒', '确认收货嘛？','确认', '取消', function(flag){
		//确认收货
		$.ajax({
			url			:		contextPath+"/EasybuyOrderServlet",
			method		:		"post",
			data		:		{
				action  : "confirmOrder",
				orderId	: orderId,
			...
			success     : 		function(data){
				...
				if(json.status == 1){
					...
					//成功,跳回订单页面
					if(isall == "1"){ //操作的是我的订单
						$(".m_right").load(contextPath+"/EasybuyOrderServlet?action=index&userId="+userId+"&currentPage="+currentPage+" .m_right>*");
					}else{//操作的是全部订单
						$(".m_right").load(contextPath+"/EasybuyOrderServlet?action=queryAllOrder&currentPage="+currentPage+" .m_right>*");
					}
				}else{
					mizhu.toast(json.message,1200);
				}
